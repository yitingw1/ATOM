...
def correct_attn_out(out, lses, cp_rank):
    """Correct local rank's attention output using all-gathered LSEs.
...
def cp_lse_ag_out_rs(cp_attn_out, cp_attn_lse, cp_group):
    ...
    # Workaround: CudaCommunicator.reduce_scatter has a bug when dim != 0
    # (movedim direction is wrong and result copy is a no-op). Manually
    # transpose to dim=0, reduce_scatter, then transpose back.
    out = out.movedim(1, 0).contiguous()          # [H_full, B, D]
    out = cp_group.reduce_scatter(out, dim=0)      # [H_local, B, D]
    out = out.movedim(0, 1).contiguous()           # [B, H_local, D]
    return out
